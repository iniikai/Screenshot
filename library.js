...
async function render() {
  ...
  for (const shot of visible) {
    ...
    card.querySelector('.card-meta').textContent = `${when} · ${shot.width}×${shot.height}`;
...
async function copyShot(id) {
  ...
  if (!shot) return;
  try {
    // Clipboard only accepts PNG images; captures are already PNG.
...
async function addTag(id) {
  const tag = prompt('Tag name (letters, numbers, dashes):');
  if (!tag) return;
  const clean = tag.trim().replace(/^#/, '').replace(/[^\p{L}\p{N}-]+/gu, '-').slice(0, 30);
  if (!clean) return;
  const shot = await getShot(id);
  if (!shot) return;
...
document.getElementById('export-all').addEventListener('click', async () => {
  ...
  for (const shot of [...shots].sort((a, b) => a.createdAt - b.createdAt)) {
    ...
    while (used.has(name)) name = name.replace(/\.png$/, `_${shot.id}.png`);
    used.add(name);
    entries.push({
      name,
...
autoCleanSelect.addEventListener('change', async () => {
  ...
  await chrome.storage.local.set({ autoDeleteDays: days });
